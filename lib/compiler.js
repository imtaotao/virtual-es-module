...
function hasUseEsmVars(path) {
  ...
  const hasUse = (scope) => Object.keys(scope.bindings).some(key => {
    ...
    if (kind === 'module') {
      if (referencePaths.some((item) => item === path)) {
        return true;
      }
...
function createVirtualModule(ast, importInfos, exportInfos) {
  const exportNodes = exportInfos.map(({ name, refName }) => {
    return t.objectProperty(
      t.identifier(name),
      t.arrowFunctionExpression([], t.identifier(refName)),
    );
  });
  ...
  traverse(ast, {
    ...
    ExportDeclaration(path) {
      ...
      nodes.forEach(node => {
        ...
        if (isDefault) {
          name = 'default';
          refName = __VIRTUAL_DEFAULT__
        } else {
          name = node.name ? node.name : node.id.name;
          refName = name;
        }
        exportInfos.push({ name, refName });
      })
